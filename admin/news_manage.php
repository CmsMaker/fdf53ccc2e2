...
$db->sql_query("DELETE FROM `news` WHERE `id`=$id");
...
if ($limitstart != 0) {
		$pout.= "<a title=\"&#1589;&#1601;&#1581;&#1607; &#1602;&#1576;&#1604;\" href=\"./news_manage.php".$ex."sp=".($limitstart - $max).$url."\"><<</a>&nbsp;";
}else {
	$pout .= "<<&nbsp;";
}
...
if ($db->sql_getrows("SELECT * FROM `news`") > $alpha) {
		$pout .= "&nbsp;<a title=\"&#1589;&#1601;&#1581;&#1607; &#1576;&#1593;&#1583;\" href=\"./news_manage.php".$ex."sp=".($limitstart + $max).$url."\">>></a>";
} else {
	$pout .= "&nbsp;>>";
}
$pout .= "</center></div>";
...
<script language="javascript">
	...
	{
		if(confirm("آیا می خواهید این خبر را حذف کنید؟"))
		{
...
<div class="post_box_top">
  <div class="post_box_top_text">مدیریت اخبار</div>
</div>
...
<td width="59">ID</td>
<td width="432">عنوان خبر</td>
<td width="80">بازدید ها</td>
<td width="79">عملیات</td>

</tr>
<?php
$db->sql_query("SELECT * FROM `news` ORDER BY id DESC LIMIT $limit");
...
<a href="javascript:delete_news('<?php echo $fetch['id']; ?>')" class="tooltip yellow-tooltip"><span>حذف خبر</span><div class="icon-delete"></div></a>
<a id="example1" href="pages/edit_news.php?id=<?php echo $fetch['id']; ?>" class="tooltip yellow-tooltip"><span>ویرایش خبر</span><div class="icon-edit"></div></a>
</td>
</tr>
<?php }  ?>
